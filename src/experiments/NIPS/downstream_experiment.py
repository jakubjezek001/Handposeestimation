...
def main():
    ...
    console_logger.info(
        f"Running downstream experiment for: {args.experiment_name} {args.checkpoint}"
    )
    # data preperation
    ...
    trainer = Trainer(
        ...
        heckpoint_callback=checkpoint_callback,
    )
    trainer.logger.experiment.set_code(
        overwrite=True,
        filename=os.path.join(
            MASTER_THESIS_DIR, "src", "experiments", "NIPS", "downstream_experiment.py"
        ),
    )
    ...
    save_experiment_key(
        ...
        f"{args.experiment_type}_downstream",
    )
